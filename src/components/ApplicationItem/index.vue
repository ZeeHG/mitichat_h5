  ...
  <div class="px-[22px] py-3 flex items-start border-b border-b-gap-text">
    ...
    <div class="flex justify-between items-center w-full h-full ml-4 relative">
      ...
      <div class="flex items-center">
        <img v-if="isSend" class="w-5 h-5" :src="send" alt="" />
        <span v-if="!showActionBtn" class="text-xs text-[#898989]">{{ stateStr }}</span>
        <van-button v-else size="small" hairline type="primary" @click="toDetails">{{ $t('buttons.toDetail')
        }}</van-button>
      </div>
    </div>
  </div>
...
const isSend = props.type === ApplicationTypeEnum.SentFriendApplication || props.type === ApplicationTypeEnum.SentGroupApplication
...
const showActionBtn = computed(() => {
  return props.source.handleResult === 0 && props.type === ApplicationTypeEnum.RecivedFriendApplication
})
