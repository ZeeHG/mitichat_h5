...
export function useGlobalEvent() {
  ...
  const revokedMessageHandler = ({ data }: WSEvent<RevokedInfo>) => {
    ...
    messageStore.updateQuoteMessageRevoke(data.clientMsgID);
  };
  ...
  const disposeIMListener = () => {
    ...
    // rtc
    // IMSDK.off(CbEvents.ONRECEIVENEWINVITATION, newInvitationHandler);
    // IMSDK.off(CbEvents.ONINVITEEACCEPTEDBYOTHERDEVICE, otherHandler);
    // IMSDK.off(CbEvents.ONINVITEEREJECTEDBYOTHERDEVICE, otherHandler);
    // application
