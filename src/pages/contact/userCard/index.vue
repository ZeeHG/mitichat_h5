    ...
    <div class="flex-1">
      <div class="px-[22px] py-6 mb-2 flex items-center bg-white">
        <Avatar :size="48" :src="contactStore.storeUserCardData.baseInfo?.faceURL"
          :desc="contactStore.storeUserCardData.baseInfo?.nickname" @click="avatarPreview" />
      ...
      <div class="mb-2" v-if="contactStore.storeUserCardData.groupMemberInfo && !isSelf">
        ...
        <CardDescItem v-if="showSetMuteMember" :lable="$t('setMute')" @click="$router.push('setMemberMute')" arrow />
      </div>
...
const avatarPreview = () => {
  if (contactStore.storeUserCardData.baseInfo?.faceURL?.includes('http')) {
    return showImagePreview([contactStore.storeUserCardData.baseInfo?.faceURL])
  }
}
