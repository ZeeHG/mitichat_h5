<template>
  <div class="page_container">
    ...
    <div v-if="newList.length > 0" class="flex-1 overflow-y-auto overflow-x-hidden relative">
      <AlphabetIndex :newList="newList" @scrollToIndex="scrollToIndex" />
      <virtual-list ref="vsl" class="my_scrollbar h-full overflow-y-auto" :data-key="'userID'" :data-sources="newList"
        :data-component="(item: FriendUserItem | IndexItem) => (item as IndexItem).title === undefined ? GenericListItem : LetterSection"
        :estimate-size="88" :extra-props="(item: FriendUserItem) => ({
...
renderList.value.indexList.map((_, i) => {
  newList.push({ title: renderList.value.indexList[i], userID: renderList.value.indexList[i] })
