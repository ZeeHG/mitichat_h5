<template>
  <div class="page_container">
    ...
    <van-action-sheet v-model:show="showGenderPicker" :actions="genderActions" :cancel-text="$t('buttons.cancel')"
      close-on-click-action @cancel="showGenderPicker = false" @select="genderSelect" />

    <van-action-sheet v-model:show="showBirthPicker" :cancel-text="$t('buttons.cancel')"
      @cancel="showBirthPicker = false">
      <van-date-picker v-model="currentDate" :title="$t('selectDate')" :min-date="new Date(1970, 0, 1)"
        :max-date="new Date()" @confirm="confirmDate" @cancel="showBirthPicker = false" />
    </van-action-sheet>
  </div>
</template>
