<template>
  <div class="page_container !bg-white">
    ...
    <div class="flex-1 overflow-hidden relative">
      ...
      <div v-show="loadState.initLoading" class="!absolute top-0 h-full w-full flex justify-center items-center bg-white">
...
const initMessageData = () => {
  ...
  const promiseArr = [IMSDK.getAdvancedHistoryMessageList<IAdvancedMessageResponse>(options), IMSDK.getAdvancedHistoryMessageListReverse<IAdvancedMessageResponse>(options)]
  Promise.all(promiseArr).then(res => {
    loadState.renderData = [...res[0].data.messageList, jumpMessage, ...res[1].data.messageList]
    loadState.topHasMore = res[0].data.messageList.length === 20
    loadState.bottomHasMore = res[1].data.messageList.length === 20
...
const onToBottom = () => {
  ...
  IMSDK.getAdvancedHistoryMessageListReverse<IAdvancedMessageResponse>(options)
    .then(({
      data
    }) => {
      loadState.renderData = [...loadState.renderData, ...data.messageList]
      loadState.bottomHasMore = data.messageList.length === 20
      setTimeout(() => getOffset(data.messageList.map((m: MessageItem) => m.clientMsgID), () => loadState.bottomLoading = false))
