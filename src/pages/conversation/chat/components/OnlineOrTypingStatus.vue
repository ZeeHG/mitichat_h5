<template>
  <div class="flex items-center justify-center">
    <i v-if="!typing" class="w-[6px] h-[6px] mr-1 bg-[#10CC64] rounded-full inline-block"
      :class="{ 'bg-[#999]': onlineState?.status === OnlineState.Offline }" />
    <span v-if="typing" class="text-xs text-[#999] font-normal">{{ $t('typing') }}</span>
    <span v-else class="text-xs text-[#999] font-normal">{{ platformToDetails(onlineState) }}</span>
  </div>
</template>
