<template>
  <div class="page_container">
    ...
    <div class="text-xs text-[#999] text-center pt-4 pb-1">{{ messageTimeLine }}</div>
    <div class="flex-1 overflow-y-auto">
      <NomalMessageItem v-for="(message, idx) in mergeData.multiMessage" :key="message.clientMsgID" :source="message"
        @click="clickMessageItem(message)" :noBorder="idx === mergeData.multiMessage.length - 1" />
    </div>
  </div>
</template>
...
import { MergeElem, MessageItem, PublicUserItem } from '@/utils/open-im-sdk-wasm/types/entity';
...
const endTime = mergeData.value.multiMessage[mergeData.value.multiMessage.length - 1].sendTime
const messageTimeLine = dayjs(startTime).isSame(endTime, 'day') ? dayjs(startTime).format('YYYY/M/D') : `${dayjs(startTime).format('YYYY/M.D')}-${dayjs(endTime).format('YYYY/M/D')}`;
